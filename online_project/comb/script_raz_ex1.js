  ...
  const pod ={
    data(){
      return{
        ...
        x:'<span id="rule_of_sum">Размещения.</span> Формула: <img src="formul_5.png" alt=""></p> <span v-html="x"></span>'
      }
    }
  }
    ...
    const answer ={
      data(){
        return{
          show:true,
          z:'<p>Решение.</p>',
k:"<p>Пронумеруем места в номере на кроватях (с № 1 по № 4) и будем «выдавать» каждому из трех членов команды номер места. Из 4 элементов (номеров мест) будут делаться выборки по 3 элемента, при этом важен не только состав выборки, но и порядок расположения в ней элементов (кто именно и на каком месте поедет). Число способов равно числу размещений из 4 по 3: <p>Можно рассуждать, непосредственно применяя правило произведения: для первого кадета можно выбрать любое из 4 мест, для второго - любое из 3 оставшихся, для третьего - любое из двух оставшихся, всего 4 &#215 3 &#215 2 &#215 1 = 24 способа рассадить команду в купе.</p>"
